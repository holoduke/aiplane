...
import { SKY_KEYFRAMES, computeSunDirection, sampleSkyColors, NEUTRAL_SKY_COLOR } from "../sky.js";
...
export const SKY_PRESETS = {
  ...
  mars: [
    ...
    { time: 7.0, horizon: 0x8c4620, sky: 0x5c2a0f, intensity: 0.35 },
    { time: 12.0, horizon: 0xc66b2a, sky: 0x6b3518, intensity: 0.7 },
    { time: 17.5, horizon: 0xa85428, sky: 0x4a2412, intensity: 0.5 },
    { time: 19.5, horizon: 0x6b3118, sky: 0x5c2a0f, intensity: 0.2 },
    { time: 24.0, horizon: 0x2b1508, sky: 0x5c2a0f, intensity: 0.08 },
  ],
};
...
export class EnvironmentManager {
  ...
  applyEnvironment(index) {
    const { scene, material } = this.app;
    const { shaderName, config } = this.getEnvironment(index);
    const app = this.app;
  ...
  setupSky() {
    const app = this.app;
    const { scene, texture, geometry, material } = this.app;
  ...
  updateSun() {
    const app = this.app;
    const skySample = sampleSkyColors(app.sunTime, app.skyKeyframes);
    ...
    app.ambientDirection.copy(ambientDir);

    const warmth = THREE.MathUtils.clamp(app.sunWarmth, 0.0, 1.0);
    app.ambientColor.lerpColors(
      AMBIENT_COLOR_COOL,
      AMBIENT_COLOR_WARM,
      warmth
    );
    _tmpSunColor.lerpColors(SUN_COLOR_COOL, SUN_COLOR_WARM, warmth);
    app.sunLightColor.copy(_tmpSunColor);
    ...
    app.skyTintColor.lerp(_tmpSkyAdjust, 0.35);

    if (app.terrain) {
      app.terrain.updateSun(app.sunDirection, app.currentSunIntensity);
      app.terrain.updateSunWarmth(app.sunWarmth);
    ...
    app.updateDebugLight();
    if (app.debugSunLight) {
      app.debugSunLight.color.copy(app.sunLightColor);
    }
    if (app.debugAmbientLight) {
      app.debugAmbientLight.color.copy(app.ambientColor);
    }

    if (app.material.atmosphere.uniforms.uHorizonColor) {
      app.material.atmosphere.uniforms.uHorizonColor.value.copy(
        skySample.horizonColor
      );
    }
    if (app.material.atmosphere.uniforms.uSkyColor) {
      app.material.atmosphere.uniforms.uSkyColor.value.copy(skySample.skyColor);
    }

    if (app.sunMesh) {
      const intensityTint = Math.max(0.35, app.currentSunIntensity);
      app.sunMesh.material.color
        .copy(app.sunLightColor)

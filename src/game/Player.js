...
export class Player {
  constructor(scene, camera, collisionDetector = null) {
    ...
    // Environment effects manager for terrain collision flashes
    this.environmentEffects = new EnvironmentEffectsManager(scene);
  ...
  findLowTerrainSpawn() {
    ...
    if (this.autoDescentEnabled) {
      ...
      console.log(
        `🛫 Auto-descent enabled: Starting at ${this.startAltitude}m, will descend to ${this.targetAltitude}m over ${this.descentDuration}s`
      );
    }
  ...
  createLaser(position, direction) {
    ...
    // Reset laser colors to green (in case materials were reused)
  ...
  updateLasers(deltaTime) {
    ...
    for (let i = this.lasers.length - 1; i >= 0; i--) {
      ...
      // Handle fade out effect after collision first
      if (laser.collisionTime) {
        const timeSinceCollision = currentTime - laser.collisionTime;
        const fadeProgress = timeSinceCollision / laser.fadeOutDuration;

        if (fadeProgress >= 1.0) {
          // Laser has faded out completely, remove it
          console.log("🔫💨 Laser faded out after collision");
        ...
        // Apply fade out effect with more aggressive fading and transparency
        const fadeAlpha = Math.pow(1 - fadeProgress, 3); // Even more exponential fade
        const pulse = 1 + Math.sin(currentTime * 0.01) * 0.02; // Very minimal pulsing during fade
        laser.mesh.material.emissiveIntensity = 6.0 * fadeAlpha * pulse; // Reduced intensity
        laser.glow.material.opacity = 0.2 * fadeAlpha * pulse; // Much more transparent
        laser.mesh.material.opacity = 0.6 * fadeAlpha; // Core also more transparent
      } else {
        // Check if non-collided laser has expired
      ...
      if (laser.justBounced) {
        ...
        if (collision) {
          ...
          // Bounce logic: forward hemisphere only (never backwards or downwards)
          const originalDirection = laser.direction.clone();

          // Create forward-upward hemisphere bounce
          const forwardComponent = originalDirection
            .clone()
            .setZ(0)
            .normalize(); // Forward direction without Z
          const upVector = new THREE.Vector3(0, 0, 1);

          // Random direction within forward hemisphere
          const randomAngle = Math.random() * Math.PI * 2; // 0 to 2π
          const randomRadius = Math.random() * 0.8; // 0 to 0.8 for spread

          const sideVector = new THREE.Vector3()
            .crossVectors(forwardComponent, upVector)
            .normalize();

          const randomHorizontal = forwardComponent
            .clone()
            .multiplyScalar(Math.cos(randomAngle) * randomRadius)
            .add(
              sideVector
                .clone()
                .multiplyScalar(Math.sin(randomAngle) * randomRadius)
            );

          const bounceDirection = upVector
            .clone()
            .add(randomHorizontal)
            .add(forwardComponent.multiplyScalar(0.3)) // Always some forward momentum
            .normalize();
          ...
          // Set collision time for fade out effect
          if (!laser.collisionTime) {
            laser.collisionTime = currentTime;
            laser.fadeOutDuration = 150 + Math.random() * 50; // 150-200ms

            // Change laser color to red for debugging
            laser.mesh.material.color.setHex(0xff0000);
            laser.mesh.material.emissive.setHex(0xff0000);
            laser.glow.material.color.setHex(0xff4400);
            laser.glow.material.emissive.setHex(0xff4400);
          }
      ...
      // Apply visual effects based on collision state
      if (laser.collisionTime) {
        // Already handled collision fade-out above, skip normal effects
      } else {
        // Normal pulsing effect for non-collided lasers
        const pulse = 1 + Math.sin(currentTime * 0.01) * 0.2;
        laser.mesh.material.emissiveIntensity = 8.0 * pulse;
        laser.glow.material.opacity = 0.4 * pulse;
      }
    }
  }

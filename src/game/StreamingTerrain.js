...
export class StreamingTerrain {
  ...
  smoothNoise(x, z, octaves = 4, persistence = 0.4, scale = 1, amplitude = 1) {
    ...
    for (let i = 0; i < octaves; i++) {
      value += this.noise(x * frequency, z * frequency, i * 137) * currentAmplitude
  ...
  generateHeightAt(worldX, worldZ) {
    ...
    const mountainRange1 = this.createMountainRange(worldX, worldZ, 0, 0, 1400, 0.0001)
    const mountainRange2 = this.createMountainRange(worldX, worldZ, 8000, 5000, 1200, 0.00012)
    const mountainRange3 = this.createMountainRange(worldX, worldZ, -6000, 8000, 1100, 0.00015)
    
    // Take the maximum for distinct mountain ranges
    height += Math.max(mountainRange1, mountainRange2 * 0.9, mountainRange3 * 0.8)
    
    // Valley systems that carve through terrain
    const valley1 = this.createValley(worldX, worldZ, 2000, -2000, 300, 0.0002)
    const valley2 = this.createValley(worldX, worldZ, -4000, 3000, 250, 0.00018)
  ...
  generateChunk(chunkX, chunkZ) {
    ...
    for (let z = 0; z < this.chunkResolution; z++) {
      for (let x = 0; x < this.chunkResolution; x++) {
        const worldX = worldPos.x + (x / (this.chunkResolution - 1) - 0.5) * this.chunkSize
        const worldZ = worldPos.z + (z / (this.chunkResolution - 1) - 0.5) * this.chunkSize
    ...
    if (this.chunks.size < 3) {
      console.log(`Chunk ${chunkX},${chunkZ} height range: ${minHeight.toFixed(1)} to ${maxHeight.toFixed(1)}`)
    }

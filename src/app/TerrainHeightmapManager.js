...
import { getTerrainTexture, regenerateNoise } from "../noise/index.js";
import * as THREE from "three";

/**
 * Manages heightmap data and terrain effects with reference-based system
 * for bidirectional erosion slider support
 */
export class TerrainHeightmapManager {
  constructor() {
    ...
    // Legacy heightmap storage (for compatibility)
    this.originalHeightmap = null;
    this.rawHeightmap = null;
  }

  /**
   * Initialize base and master heightmap from current terrain texture
   */
  initializeMasterHeightmap(type = 'procedural', generatedHeightTexture = null) {
    if (generatedHeightTexture) {
      console.log('🎯 Initializing master heightmap from generated texture (.bin file)');
      this.baseHeightmapType = 'generated';
      // TODO: Extract data from generatedHeightTexture
      return;
    }
  ...
  applyHeightmapEffect(effectType, parameters = {}) {
    ...
    if (['ridged', 'cellular', 'billow', 'warped'].includes(effectType)) {
      ...
      this.updateEffectParameters(effectType, parameters);
    }
  ...
  /**
   * Update effect parameters for specific effect type
   */
  ...
  /**
   * Generate raw Perlin noise for legacy compatibility
   */
  storeRawHeightmap(noiseResolution) {
    // Generate raw Perlin noise without height multiplier or smoothing
    const width = noiseResolution;
    const height = noiseResolution;
  ...
  /**
   * Store original heightmap from current terrain texture (legacy)
   */
  ...
  /**
   * Check if heightmap system is initialized
   */
  isInitialized() {
  ...
  /**
   * Get current heightmap dimensions
   */
  getDimensions() {
    return {
  ...
  /**
   * Get copy of master heightmap data
   */
  getMasterHeightmapCopy() {
    return this.masterHeightmap ? this.masterHeightmap.slice() : null;
  }
}